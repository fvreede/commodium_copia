    ...
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-2 md:p-4 bg-slate-50">
        ...
        <article v-for="product in products" :key="product.id" class="bg-beige shadow-md rounded-lg p-2 md:p-4 flex items-start justify-between">
            <div class="flex-1 pr-4">
                ...
                <Link :href="getProductLink(product)" class="inline-block bg-orange-600 hover:bg-orange-700 text-white text-sm md:text-base font-medium py-2 px-4 rounded-md shadow-lg transition duration-300 hover:-translate-y-1 hover:shadow-xl">
                    Bekijk product
                </Link>
            </div>
 ...
 * @returns {string} URL voor de productpagina.
 */
const getProductLink = (product) => {
    ...
    return `/products/${product.id}?${queryParams}`;
};
